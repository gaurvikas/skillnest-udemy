    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600 dark:text-gray-400 mt-1">
                {{ __('Manage registered Course Review') }}
            </p>
        </div>

        <x-button tag="a" href="{{ route('admin.course-reviews.create') }}" icon="fas-plus">
            {{ __('Add Course Review') }}
        </x-button>
    </div>
    ...
    <div
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                @forelse ($CourseReviews as $CourseReview)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                        ...
                        <td class="px-6 py-4 text-sm font-medium text-gray-800 dark:text-gray-100">
                            {{ $CourseReview->user->name ?? 'N/A' }}
                        </td>

                        <!-- Course -->
                        <td class="px-6 py-4 text-sm font-medium text-gray-800 dark:text-gray-100">
                            {{ $CourseReview->course->title ?? 'N/A' }}
                        </td>

                        <!-- rating -->
                        <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                            {{ $CourseReview->rating ?? 'N/A'}}
                        </td>
                        ...
                        <td class="px-6 py-4 text-sm flex justify-end gap-2">
                            <x-button tag="a" href="{{ route('admin.course-reviews.show', $CourseReview) }}" type="info"
                                icon="fas-eye">View </x-button>

                            <x-button tag="a" href="{{ route('admin.course-reviews.edit', $CourseReview) }}" type="warning"
                                icon="fas-pencil">Edit</x-button>

                            <form action="{{ route('admin.course-reviews.destroy', $CourseReview) }}" method="POST"
